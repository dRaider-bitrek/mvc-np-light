...
//    private final SimpleChatServerHandler simpleChatServerHandler;
    private final CommandParserHandler commandParserHandler;
...
//        pipeline.addLast(new DelimiterBasedFrameDecoder(1024 * 1024, Delimiters.lineDelimiter()));
        ...
        pipeline.addLast("idleStateHandler", new IdleStateHandler(230, 0, 0));
        pipeline.addLast(new LineBasedFrameDecoder(2048));

        pipeline.addLast(stringDecoder);
        pipeline.addLast(stringEncoder);
        pipeline.addLast(commandParserHandler);
